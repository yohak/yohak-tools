...
describe("extractCurrentTranslate", () => {
  it("should find translateValue", () => {
    const result = extractCurrentTransform("translate3d(0, 20px, 40px) scale(1,1)");
  ...
  it("should find scale value", () => {
    const result = extractCurrentTransform("translate3d(0, 20px, 40px) scale(1,0.5)");
    expect(result.scaleX).toBe("1");
    expect(result.scaleY).toBe("0.5");
  });
  it("should find skew value", () => {
    const result = extractCurrentTransform("translate3d(0, 20px, 40px) skew(0, 50deg)");
